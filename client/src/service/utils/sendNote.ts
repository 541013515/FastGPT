...
const emailMap: { [key: string]: any } = {
  [UserAuthTypeEnum.register]: {
    subject: '注册  小白屋 GPT 账号',
    html: (code: string) => `<div>您正在注册 小白屋 GPT 账号，验证码为：${code}</div>`
  },
  [UserAuthTypeEnum.findPassword]: {
    subject: '修改 小白屋GPT 密码',
    html: (code: string) => `<div>您正在修改 小白屋 GPT 账号密码，验证码为：${code}</div>`
  }
};
